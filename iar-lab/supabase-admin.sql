...
-- The question every policy asks, answered in ONE place. It must be SECURITY
-- DEFINER: a policy that reads public.admins to decide who may read
-- public.admins recurses, and Postgres refuses the whole query. That mistake
-- silently told every allowlisted person they were not on the list.
